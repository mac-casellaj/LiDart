...
& H2-4 & False positive that the state is correctly estimated & State estimation filtering error & 1. The 3D model output is incorrect & Visual inspection of 3D model output & 1. Automatically save data so that the scans performed prior to incorrect localization are not lost \newline 2. Re-scan the affected area & \srref{SR11} \\\cline{2-8}
